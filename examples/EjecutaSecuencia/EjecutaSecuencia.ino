/*
    Versión completa del robot escornabot sin comunicación Bluetooth. Se selecciona la secuencia mediante los pulsadores y
    esta es ejecutada al presionar el botón central (S5).
    Lecturas esperadas:
        S1: 512
        S2: 675
        S3: 767
...
#define MAXIMOS_MOVIMIENTOS 20   // Maximo numero de movimientos a memorizar por el robot
#define DISTANCIA_AVANCE 10      // Distancia en cm a recorrer en cada avance

// Inicializamos la clase Escornabot para hacer uso de sus funciones y variables
Escornabot Escornabot(10);

int vector_movimientos[MAXIMOS_MOVIMIENTOS];  // Vector que almacena hasta 20 movimientos
int indice_movimiento = 0;                    // Variable que guarda la posicion a almacenar en el vector
...
void ejecuta_secuencia(const int vector_movimientos[MAXIMOS_MOVIMIENTOS], int indice_movimiento) {
    ...
    for (int i = 0 ; i < indice_movimiento ; i++) {
        ...
        switch (vector_movimientos[i]) {
            ...
            case 4: // Mover a izda (rueda derecha adelante y rueda izda hacia atras)
...
void loop() {
    ...
    if (indice_movimiento < MAXIMOS_MOVIMIENTOS) {
        ...
        if ((Escornabot.S1_valor_minimo < lectura_pulsador) && (lectura_pulsador < Escornabot.S1_valor_maximo)) {
            ...
            vector_movimientos[indice_movimiento] = 1; // Almacenamos un 1 en la secuencia de movimientos
            indice_movimiento = indice_movimiento + 1; // Incrementamos la posición del vector (para almacenar siguiente valor de la secuencia)
        }
        ...
        // Comprobamos la lectura dentro del rango del boton S2
            ...
            vector_movimientos[indice_movimiento] = 2; // Almacenamos un 2 en la secuencia de movimientos
            indice_movimiento = indice_movimiento + 1; // Incrementamos la posición del vector (para almacenar siguiente valor de la secuencia)
        }
        ...
        // Comprobamos la lectura dentro del rango del boton S3
            ...
            vector_movimientos[indice_movimiento] = 3; // Almacenamos un 3 en la secuencia de movimientos
            indice_movimiento = indice_movimiento + 1; // Incrementamos la posición del vector (para almacenar siguiente valor de la secuencia)
        }
        ...
        // Comprobamos la lectura dentro del rango del boton S4
            ...
            vector_movimientos[indice_movimiento] = 4; // Almacenamos un 4 en la secuencia de movimientos
            indice_movimiento = indice_movimiento + 1; // Incrementamos la posición del vector (para almacenar siguiente valor de la secuencia)
        }
    ...
    if ((Escornabot.S5_valor_minimo < lectura_pulsador) && (lectura_pulsador < Escornabot.S5_valor_maximo)) {
        ...
        ejecuta_secuencia(vector_movimientos, indice_movimiento);   // Ejecutamos toda la secuencia de movimientos
        indice_movimiento = 0;                                      // Reiniciamos la secuencia de movimientos
        Escornabot.tono_fin (8000, 100, 100);                       // Generamos el tono final
    }
